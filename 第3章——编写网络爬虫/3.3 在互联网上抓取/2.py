...
def getAllExternalLinks(siteUrl):
    resp=requests.get(url=siteUrl,headers=headers,proxies=proxies)
    bs=BeautifulSoup(resp.text,'html.parser')

    domain=f'{urlparse(siteUrl).scheme}://{urlparse(siteUrl).netloc}'
    #domain = 'https://en.wikipedia.org'类似
    internalLinks=getInternalLinks(bs,domain)
    externalLinks=getExternalLinks(bs,domain)

    for link in externalLinks:
        if link not in allExtLinks:#如果没有访问过
            allExtLinks.add(link)
            print(f'即将获取链接的外页面是：{link}')
    for link in internalLinks:
        if link not in allIntLinks:
            print(f'即将获取链接的内页面是：{link}')
